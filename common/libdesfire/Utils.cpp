...
void Utils::PrintHexBuf(const uint8_t* u8_Data, const uint32_t u32_DataLen, const char* s8_LF, int s32_Brace1, int s32_Brace2)
{
...
void Utils::PrintInterval(uint64_t u64_Time, const char* s8_LF)
{
    ...
    cout << s32_Days << " days, " << s32_Hour << ':' << s32_Min << " hours";
    if (s8_LF) Print(s8_LF);
}
...
void Utils::XorDataBlock(uint8_t* u8_Out, const uint8_t* u8_In, const uint8_t* u8_Xor, int s32_Length)
{
...
void Utils::XorDataBlock(uint8_t* u8_Data, const uint8_t* u8_Xor, int s32_Length)
{
    for (int B=0; B<s32_Length; B++)
    {
        u8_Data[B] ^= u8_Xor[B];
...
void Utils::RotateBlockLeft(uint8_t* u8_Out, const uint8_t* u8_In, int s32_Length)
{
    int s32_Last = s32_Length -1;
    std::memcpy(u8_Out, u8_In + 1, s32_Last);
...
void Utils::GenerateRandom(uint8_t* u8_Random, int s32_Length)
{
...
uint16_t Utils::CalcCrc16(const uint8_t* u8_Data, int s32_Length)
{
...
uint32_t Utils::CalcCrc32(const uint8_t* u8_Data1, int s32_Length1, // data to process
                          const uint8_t* u8_Data2, int s32_Length2) // optional additional data to process (these parameters may be omitted)
{
...
// private
uint32_t Utils::CalcCrc32(const uint8_t* u8_Data, int s32_Length, uint32_t u32_Crc)
{
    for (int i=0; i<s32_Length; i++)
    {
        u32_Crc ^= u8_Data[i];
