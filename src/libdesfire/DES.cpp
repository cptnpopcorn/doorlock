...
bool DES::SetKeyData(const uint8_t* u8_Key, int s32_KeySize, uint8_t u8_Version)
{
...
// 1 block = 8 bytes
bool DES::CryptDataBlock(uint8_t u8_Out[8], const uint8_t u8_In[8], DESFireCipher e_Cipher)
{
    switch (ms32_KeySize)
    {
        case 8: // simple DES
...
void DES::StoreKeyVersion(uint8_t* u8_KeyOut, const uint8_t* u8_KeyIn, int s32_KeySize, uint8_t u8_Version)
{
    uint8_t u8_Mask = 0x80;
    for (int i=0; i<8; i++)
    {
        uint8_t u8_Parity = (u8_Version & u8_Mask) ? 0x01 : 0x00;
