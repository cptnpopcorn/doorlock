...
public:
    ...
    virtual bool SetKeyData(const uint8_t* u8_Key, int s32_KeySize, uint8_t u8_Version) = 0;
    virtual bool CryptDataBlock(uint8_t* u8_Out, const uint8_t* u8_In, DESFireCipher e_Cipher) = 0;
    ...
    bool CryptDataCBC(DESFireCBC e_CBC, DESFireCipher e_Cipher, uint8_t* u8_Out, const uint8_t* u8_In, int s32_ByteCount)
    {
        ...
        for (int B=0; B<s32_ByteCount/ms32_BlockSize; B++)
        {
            if (e_CBC == CBC_SEND)
            {
                ...
                std::memcpy(mu8_IV, u8_Out, ms32_BlockSize);
            }
            else // CBC_RECEIVE
            {
                ...
                std::memcpy(mu8_IV, u8_In,   ms32_BlockSize);                       // Step 2 (mu8_IV can be changed now, u8_In has not yet been modified)
                std::memcpy(u8_Out, u8_Temp, ms32_BlockSize);                       // Step 3 (here also u8_In is modified if u8_Out and u8_In are the same buffer)
            }
    ...
    bool GenerateCmacSubkeys()
    {
        ...
        std::memcpy (mu8_Cmac1, u8_Data, ms32_BlockSize);
        Utils::BitShiftLeft(mu8_Cmac1, ms32_BlockSize);
        if (u8_Data[0] & 0x80)
            mu8_Cmac1[ms32_BlockSize-1] ^= u8_R;
        
        std::memcpy (mu8_Cmac2, mu8_Cmac1, ms32_BlockSize);
    ...
    bool CalculateCmac(TxBuffer& i_Buffer, uint8_t u8_Cmac[16])
    {
    ...
    { 
        return std::max(s32_MinSize, ms32_KeySize); 
    }   
    ...
    inline void ClearIV() 
    {
        std::memset(mu8_IV, 0, ms32_BlockSize);
    }
...
protected:
    uint8_t mu8_IV[16];  // Initialization Vector for CBC
    uint8_t mu8_Key[24];
    int  ms32_KeySize;
    int  ms32_BlockSize;
    uint8_t mu8_Version;
    ...
    uint8_t mu8_Cmac1[16]; // CMAC subkey 1
    uint8_t mu8_Cmac2[16]; // CMAC subkey 2
};
