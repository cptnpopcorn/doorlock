...
**************************************************************************/
bool Desfire::Authenticate(uint8_t u8_KeyNo, DESFireKey* pi_Key)
{
    if (mu8_DebugLevel > 0)
    {
        char s8_Buf[80];
    ...
    uint8_t u8_RndB_enc[16]; // encrypted random B
    ...
    uint8_t u8_RndB[16];  // decrypted random B
    ...
    uint8_t u8_RndB_rot[16]; // rotated random B
    ...
    uint8_t u8_RndA_enc[16]; // encrypted random A
...
**************************************************************************/
bool Desfire::ChangeKey(uint8_t u8_KeyNo, DESFireKey* pi_NewKey, DESFireKey* pi_CurKey)
{
    if (mu8_DebugLevel > 0)
    {
        char s8_Buf[80];
    ...
    uint8_t u8_Command[] = { DF_INS_CHANGE_KEY, u8_KeyNo };   
    ...
    uint8_t u8_Cryptogram_enc[40] = {0}; // encrypted cryptogram
...
**************************************************************************/
bool Desfire::GetKeyVersion(uint8_t u8_KeyNo, uint8_t* pu8_Version)
{
    char s8_Buf[80];
    if (mu8_DebugLevel > 0)
    {
...
bool Desfire::GetCardVersion(DESFireCardVersion* pk_Version)
{
    ...
    uint8_t* pu8_Ptr = (uint8_t*)pk_Version;
...
**************************************************************************/
bool Desfire::GetKeySettings(DESFireKeySettings* pe_Settg, uint8_t* pu8_KeyCount, DESFireKeyType* pe_KeyType)
{
    if (mu8_DebugLevel > 0) Utils::Print("\r\n*** GetKeySettings()\r\n");
  
    uint8_t u8_RetData[2];
...
**************************************************************************/
bool Desfire::GetRealCardID(uint8_t u8_UID[7])
{
    if (mu8_DebugLevel > 0) Utils::Print("\r\n*** GetRealCardID()\r\n");
...
**************************************************************************/
bool Desfire::GetApplicationIDs(uint32_t u32_IDlist[28], uint8_t* pu8_AppCount)
{
    if (mu8_DebugLevel > 0) Utils::Print("\r\n*** GetApplicationIDs()\r\n");
    ...
    *pu8_AppCount = (s32_Read1 + s32_Read2) / 3;
    ...
    for (uint8_t i=0; i<*pu8_AppCount; i++)
    {
    ...
    if (mu8_DebugLevel > 0)
    {
        ...
        else for (uint8_t i=0; i<*pu8_AppCount; i++)
        {
            char s8_Buf[80];
...
**************************************************************************/
bool Desfire::CreateApplication(uint32_t u32_AppID, DESFireKeySettings e_Settg, uint8_t u8_KeyCount, DESFireKeyType e_KeyType)
{
    if (mu8_DebugLevel > 0)
    {
        char s8_Buf[80];
...
**************************************************************************/
bool Desfire::GetFileIDs(uint8_t* u8_FileIDs, uint8_t* pu8_FileCount)
{
    if (mu8_DebugLevel > 0) Utils::Print("\r\n*** GetFileIDs()\r\n");
...
**************************************************************************/
bool Desfire::GetFileSettings(uint8_t u8_FileID, DESFireFileSettings* pk_Settings)
{
    if (mu8_DebugLevel > 0)
    {
        char s8_Buf[80];
...
**************************************************************************/
bool Desfire::CreateStdDataFile(uint8_t u8_FileID, DESFireFilePermissions* pk_Permis, int s32_FileSize)
{
    if (mu8_DebugLevel > 0)
    {
        char s8_Buf[80];
...
**************************************************************************/
bool Desfire::DeleteFile(uint8_t u8_FileID)
{
    if (mu8_DebugLevel > 0)
    {
        char s8_Buf[80];
...
**************************************************************************/
bool Desfire::ReadFileData(uint8_t u8_FileID, int s32_Offset, int s32_Length, uint8_t* u8_DataBuffer)
{
    if (mu8_DebugLevel > 0)
    {
        char s8_Buf[80];
    ...
    while (s32_Length > 0)
    {
        int s32_Count = std::min(s32_Length, 48); // the maximum that can be transferred in one frame (must be a multiple of 16 if encryption is used)
...
**************************************************************************/
bool Desfire::WriteFileData(uint8_t u8_FileID, int s32_Offset, int s32_Length, const uint8_t* u8_DataBuffer)
{
    if (mu8_DebugLevel > 0)
    {
        char s8_Buf[80];
    ...
    while (s32_Length > 0)
    {
        int s32_Count = std::min(s32_Length, MAX_FRAME_SIZE - 8); // DF_INS_WRITE_DATA + u8_FileID + s32_Offset + s32_Count = 8 bytes
...
**************************************************************************/
bool Desfire::ReadFileValue(uint8_t u8_FileID, uint32_t* pu32_Value)
{
	TX_BUFFER(i_Params, 1);
	i_Params.AppendUint8(u8_FileID);
...
uint8_t Desfire::GetLastPN532Error()
{
...
**************************************************************************/
int Desfire::DataExchange(uint8_t u8_Command, TxBuffer* pi_Params, uint8_t* u8_RecvBuf, int s32_RecvSize, DESFireStatus* pe_Status, DESFireCmac e_Mac)
{
...
int Desfire::DataExchange(TxBuffer* pi_Command,               // in (command + params that are not encrypted)
                          ...
                          uint8_t* u8_RecvBuf, int s32_RecvSize, // out
                          ...
                          DESFireCmac    e_Mac)               // in
{
    ...
    uint8_t u8_Command = pi_Command->GetData()[0];

    uint8_t u8_CalcMac[16];
    ...
    std::memcpy(mu8_PacketBuffer + P, pi_Command->GetData(), pi_Command->GetCount());
    P += pi_Command->GetCount();

    std::memcpy(mu8_PacketBuffer + P, pi_Params->GetData(),  pi_Params->GetCount());
    P += pi_Params->GetCount();

    if (!SendCommandCheckAck(mu8_PacketBuffer, P))
        return -1;

    uint8_t s32_Len = ReadData(mu8_PacketBuffer, s32_RecvSize + s32_Overhead);
...
bool Desfire::CheckCardStatus(DESFireStatus e_Status)
{
    ...
    switch (e_Status)
    {
        ...
        default:
            Utils::Print("0x");
            Utils::PrintHex8((uint8_t)e_Status, LF);
            return false;
    }
}
...
bool Desfire::Selftest()
{
    ...
    const uint8_t u8_KeyA[24] = { 0x00, 0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80, 0x90, 0xA0, 0xB0, 0xB0, 0xA0, 0x90, 0x80, 0x70, 0x60, 0x50, 0x40, 0x30, 0x20, 0x10, 0x00 };
    const uint8_t u8_KeyB[24] = { 0x10, 0x18, 0x20, 0x28, 0x30, 0x38, 0x40, 0x48, 0x50, 0x58, 0x60, 0x68, 0x70, 0x78, 0x80, 0x88, 0x90, 0x98, 0xA0, 0xA8, 0xB0, 0xB8, 0xC0, 0xC8 };
    ...
    uint8_t u8_FileIDs[32];
    uint8_t u8_FileCount;
