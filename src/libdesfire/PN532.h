...
public:
	...
	bool SamConfig();
	bool GetFirmwareVersion(uint8_t *pIcType, uint8_t *pVersionHi, uint8_t *pVersionLo, uint8_t *pFlags);
	...
	// ISO14443A functions
	bool ReadPassiveTargetID(uint8_t *uidBuffer, uint8_t *uidLength, eCardType *pe_CardType);

protected:
	// Low Level functions
	bool CheckPN532Status(uint8_t u8_Status);
	bool SendCommandCheckAck(uint8_t *cmd, uint8_t cmdlen);
	uint8_t ReadData(uint8_t *buff, uint8_t len);

	uint8_t mu8_DebugLevel; // 0, 1, or 2
	uint8_t mu8_PacketBuffer[PN532_PACKBUFFSIZE];
